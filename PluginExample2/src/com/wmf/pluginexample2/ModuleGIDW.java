...
public class ModuleGIDW extends ModuleBase
{
	public ModuleGIDW(int nID)
	{
		super(nID);
		...
		m_strField = "";
		m_strZField = "";
		m_stra = "-0.0974887 -0.184238 -0.0047487";
		...
		m_strDSRaster = "";
		m_strDatasetRaster = String.format("GIDW%d", m_nID);
    }
	...
	public boolean OnAttach(Port portFrom, Port portTo)
	{
		try
		{
			...
			{
				m_strDSDEM = (String)alParam.get(0).m_objValue;
				m_strDatasetDEM = (String)alParam.get(1).m_objValue;
			}
	...
	public int GetParam(ArrayList<Param> alParam)
	{
		try
		{
			...
			alParam.add(new Param(Param.Datasource, m_strDSDEM, "数据源", "DEM数据源别名", "输入端", Param.EditType.ReadOnly));
			alParam.add(new Param(Param.Dataset, m_strDatasetDEM, "数据集", "DEM数据集名称", "输入端", Param.EditType.ReadOnly));
			
			alParam.add(new Param(Param.Field, m_strField, "插值字段", "", "参数", Param.EditType.SelectOrInput, strOptionalValues));
			alParam.add(new Param("ZField", m_strZField, "高程字段", "", "参数", Param.EditType.SelectOrInput, strOptionalValues));
			alParam.add(new Param("a", m_stra, "系数", "经度系数 纬度系数 高程系数", "参数", Param.EditType.FixedFloatArray));
	...
	public int SetParam(final ArrayList<Param> alParam)
	{
		...
		try
		{
			...
			m_strDSDEM = (String)alParam.get(i++).m_objValue;
			m_strDatasetDEM = (String)alParam.get(i++).m_objValue;
			
			m_strField = (String)alParam.get(i++).m_objValue;
			m_strZField = (String)alParam.get(i++).m_objValue;
			m_stra = (String)alParam.get(i++).m_objValue;
			...
			m_alInputPort.get(1).SetName(m_strDatasetDEM + "@" + m_strDSDEM);
	...
	public boolean Execute()
	{
		try
		{
			...
			if (!model.CheckInput(this, 0, m_strDSPoint, m_strDatasetPoint, false))
				return false;
			if (!model.CheckInput(this, 1, m_strDSDEM, m_strDatasetDEM, false))
				return false;
			Workspace ws = model.m_ws;
			
			Analyst analyst = Analyst.CreateInstance("GIDW", ws);
	        ...
	        str = "{\"Datasource\":\"" + m_strDSDEM + "\",\"Dataset\":\"" + m_strDatasetDEM + "\"}";
	        analyst.SetPropertyValue("DEM", str);
	        analyst.SetPropertyValue("ZField", m_strZField); //高程字段
	        analyst.SetPropertyValue("a", m_stra.replace(" ", ",")); //经度系数,纬度系数,高程系数
